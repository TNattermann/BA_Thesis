...
from ModelTraining._utils.ada_inference import infere_ada_002
import glob
...
class TransferReal:
    ...
    def main(self):
        ...
        get_combi(df=m_pred, column='prediction', title='CTR and Total Impressions for Complete Emotions', xlabel='predictions', sort=True) 
        print('Counts for Impressions')
        print(m_pred['prediction'].value_counts())
        get_barplot(df=m_pred, column='prediction', title='Total Impressions for Complete Emotions') 
        X_preds = pd.DataFrame(X_preds).rename(columns={0: 'prediction'})
        print('Counts for Referrer')
        print(X_preds['prediction'].value_counts())
        get_barplot(df=X_preds, column='prediction', title='Predicted Class Distribution for Complete Emotions') 
    ...
    def load_ml_model(self, X):
        ...
        checkpoint_filepath = f'Results/checkpoints/ckpt_model_basic.h5' 
        ...
        X_preds = pd.DataFrame(X_proba).idxmax(axis=1).map({v: k for k, v in CLASSMAPPING_5.items()}) 
        max_index = pd.DataFrame(X_proba).idxmax(axis=1)
        condition = pd.DataFrame(X_proba).max(axis=1) >= 0.6
        X_preds_certain = max_index[condition].map({v: k for k, v in CLASSMAPPING_5.items()}) 
